...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🤨 JIR COEG DI PAUSE SAMA {CallbackQuery.from_user.mention}!",
    ...
    if command == "resumecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🤨 RESUME APA MESUM? {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"🤨 NGAPA DI MATIIN? by {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if not _playlist:
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                cap = f"🎧__Playing:__ [{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡__Info:__</b> [Get Here](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n__[☕Contact Owner](https://t.me/zenzuzu2)__\n🎧*__Requested by:__* {CallbackQuery.from_user.mention}"
                final_output = await CallbackQuery.message.reply_photo(
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=cap,
